...
```bash
openclaw models auth --agent dev login --provider openai-codex --set-default
```

Because the sandbox runs headless, the supported flow is manual redirect copy-paste:

1. Copy the authorization URL printed by `openclaw`.
...
4. Paste that redirect URL back into the CLI prompt inside the sandbox.
...
The bundled `policy.yaml` includes an `openai_codex` network policy for the minimum endpoints needed by this flow:
