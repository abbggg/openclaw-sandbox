...
#   3. this helper keeps Telegram in pairing mode so only explicitly approved
#      DMs can talk to the bot
set -euo pipefail
...
openclaw config set channels.telegram.execApprovals.enabled false --strict-json
...
openclaw config unset channels.telegram.execApprovals.approvers >/dev/null 2>&1 || true
openclaw config unset channels.telegram.execApprovals.agentFilter >/dev/null 2>&1 || true
openclaw config unset channels.telegram.execApprovals.sessionFilter >/dev/null 2>&1 || true
openclaw config unset channels.telegram.execApprovals.target >/dev/null 2>&1 || true
...
echo "  Native exec approvals: disabled"
echo ""
echo "Next:"
